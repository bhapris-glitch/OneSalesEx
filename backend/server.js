...
app.post('/api/chat/message', async (req, res) => {
  try {
    ...
    let reply = 'I can help you discover products, compare options, and find today’s best offer?';
    if (process.env.OPENAI_API_KEY) {
      const response = await fetch('https://api.openai.com/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model: trial || plan === 'premium' ? plans.premium.model : (plans[plan]?.model || 'gpt-4o-mini'), temperature: 0.4, max_tokens: 350, messages: [{ role: 'system', content: `You are ${settings.agentName}, the sales executive for ${settings.storeName}. ${settings.behavior} Help shoppers choose products and encourage a purchase without making up product details.` }, { role: 'user', content: String(req.body.message || '').slice(0, 2000) }] }) });
